...
export default function Dashboard() {
    ...
    // Onboarding tour — dispara uma vez quando o usuário tem apenas 1 evento
    useEffect(() => {
        if (loading || events.length !== 1 || tourFired.current) return;
        if (localStorage.getItem('lume-tour-done')) return;
        ...
        const timer = setTimeout(() => {
            const driverObj = driver({
                ...
                popoverClass: 'lume-tour-popover',
                steps: [
                    {
                        element: '#tour-qr',
                        popover: {
                            title: '🎉 Seu evento está no ar!',
                            description: 'Este é o QR Code do seu evento. Imprima e coloque em mesas — seus convidados escaneiam e a câmera abre direto no celular deles.',
                    ...
                    },
                    {
                        ...
                        popover: {
                            ...
                            description: 'Copie o link e envie via WhatsApp, Instagram ou e-mail para seus convidados acessarem mesmo sem o QR Code.',
                    ...
                    },
                    {
                        ...
                        popover: {
                            ...
                            description: 'Abra o evento para ver as fotos chegando em tempo real. Perfeito para exibir no telão durante a festa!',
                            side: 'top', align: 'center',
                        },
                    },
                ],
                onDestroyStarted: () => {
                    localStorage.setItem('lume-tour-done', '1');
                    driverObj.destroy();
                },
            });
            driverObj.drive();
    ...
    const downloadQRCode = (eventSlug: string) => {
        ...
        img.onload = () => {
            ...
            canvas.toBlob((blob) => {
                ...
                link.href = pngUrl;
                link.download = `qr-code-${eventSlug}.png`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(pngUrl);
                toast({ title: 'QR Code baixado como PNG!' });
            }, 'image/png');
        };
        img.src = url;
    };
