...
export default function EditEvent() {
        ...
        const fetchEventData = async () => {
            ...
            try {
                ...
                form.reset({
                    ...
                    custom_message: evt.custom_message || '',
                    requires_password: evt.requires_password || false,
                    event_password: '',
                    moderation_enabled: evt.moderation_enabled || false,
    ...
    const onSubmit = async (values: EventFormValues) => {
        ...
        // Lógica de senha: só re-hash se o campo foi preenchido
        let passwordUpdate: Record<string, unknown> = {};
        if (!values.requires_password) {
            passwordUpdate = { password_hash: null };
        } else if (values.event_password?.trim()) {
            passwordUpdate = { password_hash: await hashPassword(values.event_password.trim()) };
        }

        // Remove event_password — campo de formulário, não existe no banco
        const { event_password: _pw, ...restValues } = values;
        const v = restValues as any;
        const payload = {
            ...restValues,
            name: restValues.name.trim(),
            slug: restValues.slug.trim(),
            event_date: restValues.event_date || null,
            description: restValues.description?.trim() || null,
            custom_message: restValues.custom_message?.trim() || null,
            requires_password: restValues.requires_password || false,
            moderation_enabled: restValues.moderation_enabled || false,
            ...passwordUpdate,
            couple_name_1: restValues.couple_name_1?.trim() || null,
            couple_name_2: restValues.couple_name_2?.trim() || null,
