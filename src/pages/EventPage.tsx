...
export default function EventPage() {
    ...
    useEffect(() => {
        if (event?.theme_color) applyTheme(event.theme_color);
    }, [event]);

    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--theme-light, #F5F3FF)' }}>
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-50 p-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">Evento Inativo</h2>
                    <p className="text-gray-600 mb-6">Este evento foi desativado temporariamente.</p>
                    <button
                        onClick={() => navigate('/')}
    ...
    // Verifica se o evento requer senha e se já está desbloqueado
    const isPasswordProtected = event.requires_password && event.password_hash;
    const isUnlocked = !isPasswordProtected || unlockedByUser ||
        sessionStorage.getItem(`lume-unlocked-${event.id}`) === '1';

    if (!isUnlocked) {
        return <EventPasswordGate event={event} onUnlock={() => setUnlockedByUser(true)} />;
    }
