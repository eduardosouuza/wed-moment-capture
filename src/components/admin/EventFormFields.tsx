...
import { Calendar, Sparkles, Palette, Lock, ShieldCheck } from "lucide-react";
...
export function DetailsFields({ form }: { form: UseFormReturn<EventFormValues> }) {
  ...
  const requiresPassword = useWatch({ control: form.control, name: "requires_password" });
    ...
    <Card className="border-none shadow-xl shadow-black/5 dark:bg-[#151518] edit-animate">
      ...
      <CardContent className="space-y-6">
            ...
            return (
              <FormItem className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-medium tabular-nums ${charCount > 180 ? 'text-[#E85A70]' : 'text-gray-400'}`}>
                    {charCount}/200
                  </span>
                </div>
                <FormControl>
                  <Textarea
                    placeholder="Ex: Obrigado por vir! Tire sua foto e guarde essa memória especial."
                    rows={3}
                    maxLength={200}
                ...
                <FormDescription className="text-xs text-gray-400">
                  Aparece na tela do convidado antes de tirar a foto.
                </FormDescription>
                <FormMessage />
              </FormItem>
            );
        ...
        <div className="pt-2 space-y-3">
          <p className="text-xs font-bold uppercase tracking-wider text-gray-400">Controles de Acesso</p>
            ...
            render={({ field }) => (
              <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-50/50 dark:bg-white/5 border border-[#ede7e4] dark:border-white/10">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-gray-400">Convidados precisarão inserir uma senha.</p>
                </div>
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
                      type="password"
                      placeholder="Senha do evento (mín. 4 caracteres)"
                      className="h-11 rounded-xl border-[#ede7e4] dark:border-white/10"
                      {...field}
                      value={field.value || ""}
                    />
                  </FormControl>
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-50/50 dark:bg-white/5 border border-[#ede7e4] dark:border-white/10">
                <div className="space-y-0.5">
                  <Label className="text-sm font-bold flex items-center gap-2">
                    ...
                    Aprovar fotos antes de publicar
                  </Label>
                  <p className="text-xs text-gray-400">Você revisa cada foto antes de aparecer na galeria.</p>
                </div>
                <Switch checked={field.value || false} onCheckedChange={field.onChange} />
              </div>
            )}
