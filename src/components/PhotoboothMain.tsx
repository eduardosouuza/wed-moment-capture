...
function isEventInFuture(dateStr: string | null): boolean {
  if (!dateStr) return false;
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const [y, m, d] = dateStr.split('-').map(Number);
  return new Date(y, m - 1, d, 0, 0, 0) > today;
}
...
const PhotoboothMain: React.FC<PhotoboothMainProps> = ({ event }) => {
  ...
  const [eventStarted, setEventStarted] = useState(() => !isEventInFuture(event.event_date));
  const handleCountdownExpired = useCallback(() => setEventStarted(true), []);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--theme-light, #F5F3FF)' }}>
      ...
      {event.custom_message && (
        <div className="px-6 mb-6">
          <div className="max-w-md mx-auto">
            <div
              className="px-5 py-4 rounded-2xl border text-center"
              style={{
                background: 'linear-gradient(135deg, var(--theme-light, #F5F3FF), rgba(255,255,255,0.8))',
            ...
            >
              ...
              >
                &ldquo;{event.custom_message}&rdquo;
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Countdown ou Botão Principal */}
      {!eventStarted && event.event_date ? (
        <EventCountdown
      ...
      <div className="px-4 max-w-6xl mx-auto">
        <Gallery eventId={event.id} eventName={displayName} moderationEnabled={event.moderation_enabled} />
      </div>
