...
const PLAN_LIMITS: Record<string, { max_photos: number; max_videos: number }> = {
    basico:   { max_photos: 30,   max_videos: 0 },
    standard: { max_photos: 300,  max_videos: 50 },
    premium:  { max_photos: 9999, max_videos: 9999 },
};
...
serve(async (req) => {
    ...
    try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            // AUTHENTICATED CHECKOUT - Update user plan and event limits
            try {
            ...
            // Update event payment status and limits
            const eventId = session.metadata?.event_id;
            if (eventId && PLAN_LIMITS[planType]) {
                try {
                    ...
                    const { error: eventError } = await supabase
                        ...
                        .eq('id', eventId);
                    if (eventError) {
                        console.error('⚠️ Could not update event limits (non-fatal):', eventError.message);
                    }
                } catch (dbErr: any) {
                    console.error('⚠️ DB error on event update (non-fatal):', dbErr.message);
                }
            }
